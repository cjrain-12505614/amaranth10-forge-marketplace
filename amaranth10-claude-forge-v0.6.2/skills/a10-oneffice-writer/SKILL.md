...
### Step 3. (선택) 단일페이지 모드 전환 — 웹페이지형 긴 문서일 때

A4 페이지 나눔 없이 "웹페이지처럼 길게" 붙이려면 필수. 다중페이지가 필요하면
(보고서·계약서 등) 스킵.

**UI 경로:**
```
파일 리본 → [문서 설정] → [페이지분리] → "단일 페이지" 라디오 → [확인]
```

완료 시 하단이 `1/1 페이지` 로 바뀐다.
...
**검증된 기본값 (2026-04-15, 차민수 계정, A4 세로, 여백 보통 20mm, 브라우저 줌 130%):**

| 항목 | 값 (CSS px, unzoomed) |
|---|---|
| `cssShiftLeft` | **-1** |
| `cssTargetWidth` | **644** |
| `zoom` (transform matrix) | 1.3 |
| `main.offsetWidth` | 642 |
| `padL` / `padR` | 76 / 76 |
...
### Step 8. !important 강제 제약 — 자식 요소 overflow 방지 + outline 보험

주입된 `<style>` 태그 안에 또는 별도 JS 주입으로 다음 규칙 추가:

```css
/* ONEFFICE outline 제거 — Step 4에서 HTML에 이미 포함됐어야 함. DOM 주입 후 보험으로도 추가 */
* { outline: none !important; }
...
(() => {
  const iframe = document.getElementById('open_oneffice_body_iframe');
  const editorDoc = iframe.contentDocument.getElementById('dzeditor_0').contentDocument;
  const styleTag = editorDoc.createElement('style');
...
| 스타일 대상 (모드 B, 권장) | `main > .container` |
| 스타일 대상 (모드 A, 레거시) | `main > div.main` (rootDiv) |
...
| 기본 프리셋 style | `margin:0 0 0 -1px;width:644px;max-width:644px;padding:0;box-sizing:border-box;` |
