...
description: >
  ...
  "depth 일관성 검증", "정기 점검", "Step 진입 직전 검증",
  or 워크스페이스 광역 정합 점검 필요 시.
  깨진 링크 + 옛 경로 잔존 + 산출물 위치 정합 + depth-up 정합 4축 점검.
  결과 리포트 → 참고자료/리포트/residual-audit-{YYYYMMDD}.md.
version: 0.1.0
---

# 잔존 검증 (a10-residual-audit) — Phase R+ R-07 신설

워크스페이스 **광역 정합 점검 4축** + 결과 리포트 자동 저장. R-06(즉시 탐지)의 광역 정기 점검 버전.
