...
| `_projects/_dashboard.md` | project-tracker, status-reporter, session-protocol |
| `[모듈]/module-overview.md` | context-manager, context-analyzer |
| `[모듈]/sessions/_current.md` | session-protocol, session-manager |
...
- `sessions/_current.md` — 초기 세션 체크포인트
...
`_projects/`에 새 프로젝트 파일을 생성한다.

#### Step 1: ID 채번
`_projects/_dashboard.md`에서 최신 ID를 확인하고 `PRJ-{시작연도}-{순번3자리}` 형식으로 다음 순번 부여.
...
#### Step 3: 프로젝트 파일 생성
`_projects/_templates/project-detail.md`를 복사하여 `PRJ-YYYY-NNN_{프로젝트명}.md`로 생성.
01. 프로젝트 개요와 02. TASK 목록 초안을 작성.

#### Step 4: 연쇄 업데이트 (Cascade R2)
- `_projects/_dashboard.md`에 행 추가
